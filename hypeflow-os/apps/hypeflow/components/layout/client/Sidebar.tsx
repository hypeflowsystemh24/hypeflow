...
const nav = [
  ...
  { label: 'ROI & Métricas', href: '/client/roi',       icon: TrendingUp },
  { label: 'As Minhas Leads',href: '/client/leads',     icon: Users },
  { label: 'Calls',          href: '/client/calls',     icon: Phone },
  { label: 'Pipeline',       href: '/client/pipeline',  icon: Kanban },
]

export function ClientSidebar({ clientName = 'Cliente', clientNiche = '' }: { clientName?: string; clientNiche?: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-5 py-2.5">
        <span className="w-1.5 h-1.5 rounded-full live-dot flex-shrink-0" style={{ background: 'var(--success)' }} />
      ...
      {/* Client card + logout */}
      <div className="px-3 py-4 border-t flex flex-col gap-1" style={{ borderColor: 'var(--glass-border)' }}>
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ background: 'var(--cyan-glow)' }}>
          <div
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-bold truncate" style={{ color: 'var(--t1)' }}>{clientName}</p>
            {clientNiche && <p className="tag-label" style={{ fontSize: '0.58rem' }}>{clientNiche}</p>}
          </div>
        </div>
        <button
          onClick={handleLogout}
          className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs transition-colors w-full text-left"
          style={{ color: 'var(--t3)' }}
          onMouseEnter={e => {
